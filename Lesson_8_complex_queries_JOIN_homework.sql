...
-- Задание как и в дз №6, но тут будем делать используя JOIN
...
-- Посчитаем опять же для юзера с id=1
USE vk;

SELECT 
	m.from_user_id AS Sender_id, 
	concat(u.firstname, ' ', u.lastname) AS Receiver, 
	count(*) AS message_counter 
FROM messages m
JOIN users u ON u.id=m.from_user_id	-- JOIN с таблицей юзеров
JOIN friend_requests fr ON -- JOIN с таблицей запроса в друзья (выбираем только друзей юзера с id=1)
	(fr.initiator_user_id=u.id AND fr.target_user_id=1)
	OR 
	(fr.target_user_id=u.id AND fr.initiator_user_id=1)
WHERE fr.status='approved'
GROUP BY m.from_user_id		-- Группировка по отправителю
ORDER BY count(*) DESC
LIMIT 1;
...
SELECT 
	CASE (profiles.gender)	-- для вывода пола м/ж из профилей
		WHEN 'm' THEN 'masculinum'
		WHEN 'f' THEN 'femininum'
		ELSE 'n/o'
	END AS "Gender",
	count(*)				-- счетик лайков
FROM profiles 
JOIN likes ON profiles.user_id=likes.user_id -- группировка двух таблиц по user_id
